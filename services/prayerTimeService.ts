...
export const fetchRegencies = async (): Promise<Regency[]> => {
    try {
        const url = `${BASE_URL}/regional/indonesia/prayer-times/regencies?page=2&limit=100`;
...
export const searchRegencies = async (query: string): Promise<Regency[]> => {
    ...
    try {
        ...
        const response = await fetch(`${BASE_URL}/regional/indonesia/prayer-times/regencies?page=2&limit=200`, { headers });
        const data = await response.json();
